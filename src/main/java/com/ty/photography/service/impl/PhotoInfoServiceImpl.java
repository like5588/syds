...
public class PhotoInfoServiceImpl implements IPhotoInfoService{
	...
	public void savePhotoInfo(PhotoInfo photoInfo) {
		try{
			...
			String todayNum = redisUtil.hget("upload", photoInfo.getUserId()+"::"+CommonUtils.dateToString(now));
			if(todayNum == null){
				todayNum = "0";
			}
			if(Integer.parseInt(todayNum)<15){
				//用户积分
				Date date = new Date();
				UserCoin userCoin = new UserCoin();
				userCoin.setId(CommonUtils.getUUID());
				userCoin.setUserId(photoInfo.getUserId());
				userCoin.setTotalCoin(2);
				userCoin.setCurrentCoin(2);
				userCoin.setHighestCoin(2);
				userCoin.setCreateTime(date);
				userCoin.setEarnCoin(2);
				...
				userCoinLog.setUserId(photoInfo.getUserId());
				userCoinLog.setEarnCoin(1);
				userCoinLog.setUserOperate("upload");
				userCoinLog.setCreateTime(date);
				userCoinMapper.saveLog(userCoinLog);
			}
			redisUtil.hincrby("upload", photoInfo.getUserId()+"::"+CommonUtils.dateToString(now), 1);
			redisUtil.close();
		}catch(Exception e){
			throw e;
		}
	...
	public List<PhotoInfoDto> findAllPhotos(PhotoInfoDto photoInfoDto) {
		return photoInfoMapper.findAllPhotos(photoInfoDto);
	}
