...
public class LotteryInfo{
	...
	public void init() {
		...
		for (Dictionary dictionary : probabilityList) {
			redisUtil.hset("lottery_probability",dictionary.getId(), dictionary.getDateValue());
		}
		for (Dictionary num : numList) {
			redisUtil.hset("lottery_num",num.getId(), num.getDateValue());
		}
	...
	public synchronized int lottery(String userId){
		...
		int totalTimes = Integer.parseInt(redisUtil.get("lottery_times"));
		if(times < totalTimes){
			//是否在抽奖时间范围内
			if(now.after(fromDate) && now.before(cal.getTime())){
				...
				//最多中奖数
				Map<String,String> lottery_num_map = redisUtil.hgetAll("lottery_num");
				int lottery_num_1 = Integer.parseInt(lottery_num_map.get("lottery_num_1"));
				int lottery_num_2 = Integer.parseInt(lottery_num_map.get("lottery_num_2"));
				int lottery_num_3 = Integer.parseInt(lottery_num_map.get("lottery_num_3"));
				int lottery_num_4 = Integer.parseInt(lottery_num_map.get("lottery_num_4"));
				double y = ran.nextDouble()*100;
				//概率区间
				Map<String,String> lottery_probability_map = redisUtil.hgetAll("lottery_probability");
				double lottery_probability_1 = Double.parseDouble(lottery_probability_map.get("lottery_probability_1"));
				double lottery_probability_2 = Double.parseDouble(lottery_probability_map.get("lottery_probability_2"));
				double lottery_probability_3 = Double.parseDouble(lottery_probability_map.get("lottery_probability_3"));
				double lottery_probability_4 = Double.parseDouble(lottery_probability_map.get("lottery_probability_4"));											   
				double probability_2 = (lottery_probability_1+lottery_probability_2)>=100 ? 100:(lottery_probability_1+lottery_probability_2);
				double probability_3 = (probability_2+lottery_probability_3)>=100 ? 100 :(probability_2+lottery_probability_3);
				double probability_4 = (probability_3+lottery_probability_4)>=100 ? 100 :(probability_3+lottery_probability_4);
				if(y<lottery_probability_1 && lottery_current_1<lottery_num_1){
					result = 1;
					redisUtil.incr("lottery_current_1");
				}else if(y>=lottery_probability_1 && y<probability_2 && lottery_current_2<lottery_num_2){
					result = 2;
					redisUtil.incr("lottery_current_2");
				}else if(y>=probability_2 && y<probability_3 && lottery_current_3<lottery_num_3){
					result = 3;
					redisUtil.incr("lottery_current_3");
				}else if(y>=probability_3 && y<probability_4 && lottery_current_4<lottery_num_4){
					result = 4;
