...
function init(uploadLimit,sizeLimit){
	...
	var objH=520;//浮动对象的高度
	var objW=650;//浮动对象的宽度
	var objT=Number(st)+(Number(ch)-Number(objH))/2;
	var objL=Number(sl)+(Number(cw)-Number(objW))/2;
	var uploadbody='<div id="uploadBody" class="ke-dialog-upload" style="left:'+objL+'px;top:'+objT+'px;">'
		...
		+'	<span class="ke-button-common ke-button-outer" title="确定"><input id="insertAll" class="ke-button-common ke-button" type="button" value="确定"></span>'
		+'	<span class="ke-button-common ke-button-outer" title="清空"><input id="clearAll" class="ke-button-common ke-button" type="button" value="清空"></span>'
		...
		+'	</div>'
		+' <div style="display: block; width: 100%; height: 100%; position: absolute; left: 0px; top: 0px; z-index: 811212;" class="ke-dialog-mask"></div>';
...
//	alert(swfu.getStats().successful_uploads);//successful_uploads
	var simpleGroup = $("input[name='simpleGroup']:checked").val();
	var photoGroup = $("input[name='photoGroup']:checked").val();
	var image_div;
	var num = $("img[data-status='0']").length;
	if(num >0){
		$("#form_div").hide();
	}else{
		return;
	...
	if(simpleGroup == "0"){
		$("#photo_single").show();
		$("#s_simpleGroup").val(simpleGroup);
		$("#s_photoGroup").val(photoGroup);
	}else{
		$("#photo_group").show();
		$("#g_simpleGroup").val(simpleGroup);
		$("#g_photoGroup").val(photoGroup);
	}
...
//			var title=$(obj).attr("title");
		  	if(simpleGroup == "0"){
		  		image_div = "<input type=\"hidden\" name=\"imgUrl\" value=\""+showimg.src+"\">" 
							+"<ul>" 
							+	"<li>"
							+		"<div class=\"singleimg\">"
							+			"<img src=\""+showimg.src+"\" style=\"width:230px;\"/>"
							+		"</div>"
							+	"</li>"
							+	"<li style=\"width: 450px; padding-left: 13px;\">"
							+		"<div class=\"yg_single2\">"
							+			"作品名称：<input name=\"photoTitle\" type=\"text\" class=\"yg_single3_text\" /><br /> <br />"
							+			"作品简介：<textarea name=\"photoDesc\" rows=\"2\" class=\"yg_single3_text1\"></textarea>"
							+			"<br /> <br />"
							+			"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
							+			"<input name=\"photoType_"+i+"\" type=\"radio\" value=\"1\" checked=\"checked\"/>纪实&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
							+			"<input name=\"photoType_"+i+"\" type=\"radio\" value=\"2\" /> 风景"
							+		"</div>"
							+	"</li>"
							+"</ul>";
				$("#image_form").append(image_div);
				if(i==num-1){
					$("#image_form").append("<div class=\"yg_pro_button\" style=\"margin:0 auto;\" onclick=\"submit_photos('image_form')\"></div>");
					$("#iconClose").trigger("click");
				}
		  	}else{
		  		image_div = "<input type=\"hidden\" name=\"imgUrl\" value=\""+showimg.src+"\">" 
		  					+"<li><div class=\"singleimg\"><img src=\""+showimg.src+"\" style=\"width:210px;\"/></div></li>";
		  		$("#ul_content").append(image_div);
		  		$("#iconClose").trigger("click");
		  	}
	});
