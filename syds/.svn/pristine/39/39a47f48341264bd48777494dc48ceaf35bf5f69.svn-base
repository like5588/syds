...
scrollVertical.prototype.times=3;			//滚动次数
/**
...
scrollVertical.prototype.play = function(o){
	...
	var anim = function(){
		...
		// 开始以10毫秒的时间间隔运行滚动	
		if (o.isPause) {
			o.scrollTimer = setTimeout(anim, 80);
			return;
		}
		...
		if (msg_h - o.scrollValue <= 0) {
			...
			if(currentTimes == times){
				$("#userlist").html($("#listbox_copymsgid").html());
				currentTimes = 0;
			}
		}
		else {
			o.scrollValue += 1;
			o.scrollHeight += 1;
			if(o.scrollValue == parseInt(msg_h/2) && times-currentTimes==1){
				$.ajax({ 
					type: "POST",  
					url : "recentlyInfo.do",
					cache: false, 
					...
					success: function(msg) { 
						var result = "";
						var liContent = "<li>"
							+"<div class=\"titme\">\${time}</div>"
							+"<div class=\"uername\">\${name}</div>"
							+"<div class=\"yh\">获得\${award}</div>"
							+"</li>";
						for (var one in msg) {
							var res = liContent.replace(/\${time}/g, msg[one].time);
							res = res.replace(/\${name}/g, msg[one].name);
							res = res.replace(/\${award}/g, msg[one].award);
							result += res;
						};
						$("#listbox_copymsgid").html(result);
					}
				});
			}
		}
...
function scrollVertical(disp, msg, tg){
	...
	// 获取单行的高度
	// 获取到第一(s_msg.getElementsByTagName(tg)[0])tg(一行的标签)的高度，作为单行的高度 , 另加上margin_botton的高度5
	this.unitHeight = s_msg.getElementsByTagName(tg)[0].offsetHeight+5;
	// 获取整个信息框的高度
